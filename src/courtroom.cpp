...
Courtroom::Courtroom(AOApplication *p_ao_app) : QMainWindow()
{
  ...
  connect(action_open_emote_filter_menu, &QAction::triggered, this, [this]() { 
		emoteFilterMenu = new EmoteMenuFilter(nullptr, ao_app, this);
  ...
  if (file_exists(base_path)) {
    ...
    if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        ...
        set_character_sets(get_base_path() + "global_char_set.ini");
        qDebug() << "Loaded global char set!";
    }
  }
...
void Courtroom::do_character_slide()
{
    ...
    QPoint new_pos(ui_element->centeredX, ui_element->centeredY);

    QPropertyAnimation *slide_animation = new QPropertyAnimation(ui_element, "pos", this);
...
void Courtroom::show_notification(int mode, QString display_name, QString f_message)
{
    ...
    if (mode == 0) {
        ...
        if (icon_path.isEmpty()) {
            ...
            icon_path = ":/resource/logo_ao2.png";
        }
    } else {
...
QString Courtroom::scan_for_filtered_words(QString &message) {
    QStringList list_of_words = Options::getInstance().filteredWords();
...
bool Courtroom::scan_for_callwords(QString message) {
    QStringList list_of_words = Options::getInstance().callwords();
